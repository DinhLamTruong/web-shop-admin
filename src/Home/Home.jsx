...
function Home(props) {
  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        setLoading(true);
        // Fetch both history and additional history data 
        // history: danh sách history
        // historyData: số lượng user, order
        const [history, historyData] = await Promise.all([
          HistoryAPI.getAll(),
                ...
                <div>
                  <div className="d-inline-flex align-items-center">
                    <h2 className="text-dark mb-1 font-weight-medium">
                      {!loading ? historyData.clientCount : '...'}
                    </h2>
                  </div>
              ...
              <div className="d-flex d-lg-flex d-md-block align-items-center">
                <div>
                  <h2 className="text-dark mb-1 w-100 text-truncate font-weight-medium">
                    {!loading
                      ? (+historyData.earnings).toLocaleString('vi-VN')
                      : '...'}
            ...
            <div className="card-body">
              <div className="d-flex d-lg-flex d-md-block align-items-center">
                <div>
                  <div className="d-inline-flex align-items-center">
                    <h2 className="text-dark mb-1 font-weight-medium">
                      {!loading ? historyData.orderCount : '...'}
                    </h2>
                  </div>
                  <h6 className="text-muted font-weight-normal mb-0 w-100 text-truncate">
                    New Order
                      ...
                      {history &&
                        history.map(value => (
                          <tr key={value._id}>
                            ...
                            <td>{value.phone}</td>
                            <td>{value.address}</td>
                            <td>{(+value.total).toLocaleString('vi-VN')}</td>
                            <td>
                              {value.delivery
                                ? 'Đã Vận Chuyển'
                                : 'Chưa Vận Chuyển'}
